...
class nnets:
	...
	def __init__(self,session, historySize,handSize,publicCardSize,actionSize,valueSize,alpha =1):
		...
		#Create placeholders
		self.policyNetInput=tf.placeholder(dtype=tf.float32,shape=(None,historySize+2*handSize+publicCardSize))
		...
		self.estimNetInput=tf.placeholder(dtype=tf.float32,shape=(None,historySize+handSize+publicCardSize))
...
#Functions that set properties during initialization
	...
	def getPolicyValue(self):
		...
		m = Model(input=inputs, output=[p_values , v_values])
		return m

	@define_scope
	def costPolicyValue(self):
		p , v = self.getPolicyValue(self.policyNetInput)
	...
	def getEstimateOpponent(self):
		...
		m = Model(input=inputs, output=cards)
		return m

	@define_scope
	def costEstimate(self):
		cardProb = self.getEstimateOpponent(self.estimNetInput)
...
#Auxiliary functions
	...
	def policyValue(self,playerCard, publicHistory, publicCard):
		...
		playerEstimate=np.concatenate(playerInfo,opponentCard)
		return getEstimateOpponent.eval(session = self.sess, feed_dict = {policyNetInput : playerInfo})

	def estimateOpponent(self,playerCard, publicHistory, publicCard,flattened=False):

		playerInfo=preprocessInput(playerCard, publicHistory, publicCard)

		return getEstimateOpponent.eval(session = self.sess, feed_dict = {estimNetInput : playerInfo})
	...
	def preprocessInput(self, playerCard, publicHistory, publicCard): #Method that is here only because of the input specifics
		...
		return np.concatenate(playerCard,publicHistory,publicCard)
